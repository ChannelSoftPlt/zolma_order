...
class ProductFilter extends StatefulWidget {
  final String brandid, itemnumber;
  final Function() update;

  ProductFilter({this.brandid, this.itemnumber, this.update});
...
class _ProductFilterState extends State<ProductFilter> {
  ...
  void updatecart(getid, color) async {
    DatabaseHelper.instance.querycolor(getid,color).then((value) {
      value.forEach((element) {
        ...
        final getresult = DatabaseHelper.instance.update(getid, newquantity, color);
      });
    }).catchError((error) {
      print(error);
    }).catchError((error) {
  ...
  void addcart(getid, task, setPrice, setcolor) async {
    ...
    var saveid = DatabaseHelper.instance.insert(Order(
        itemcode: getid, name: task, price: setPrice, quantity: newquantity, color: setcolor));
    taskList.insert(
        0,
        Order(
            itemcode: getid,
